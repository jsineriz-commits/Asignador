...
export async function GET() {
  try {
    ...
    // â”€â”€ Hoja "Merge Partido_Departamento" â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Hoja "BC - ID" â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Cols (0-based): A=0 [concat prov+partido], B=1 provincia, C=2 partido,
    //                 D=3 Part_Raster, E=4 ID_DEPTO
    const bcData = (bcIdRows || []).slice(1);

    // bcLookup: norm(prov) + '|' + norm(partido) â†’ DEPTO_ID
    const bcLookup = {};
    // tambiÃ©n por solo partido (fallback)
    const bcDeptOnly = {};
    ...
    // â”€â”€ Hoja "GADM - Match Deptos" â€” matches manuales (cols F + G) â”€â”€â”€
    // Col A=0 Provincia GADM, B=1 Depto GADM (NAME_2), E=4 GID_2
    // Col F=5 â†’ partido BC (partido_establecimiento_senasa)
    // Col G=6 â†’ provincia BC (para desambiguar el lookup)
    let gadmMatchRows = [];
    ...
    let manualMatches = 0;
    // gadmRawToNorm: nombre GADM crudo (col B) â†’ nombre normalizado (col D)
    const gadmRawToNorm = {};
    // gadmToRoster: nombre GADM crudo (col B) â†’ { rosterDept (col H), rosterProv (col I) }
    ...
    const result = {
      idToInfo,        // DEPTO_ID â†’ { prov, dept, lat, lng }
      nameToId,        // norm(prov)+'|'+norm(dept) â†’ DEPTO_ID
      normDeptToId,    // norm(dept) â†’ DEPTO_ID (fallback)
      bcLookup,        // norm(prov)+'|'+norm(partido) â†’ DEPTO_ID
      bcDeptOnly,      // norm(partido) â†’ DEPTO_ID (fallback)
      mergeCoords,     // [{id, lat, lng}] para matching por coordenadas con GADM
      gadmRawToNorm,   // nombre GADM crudo â†’ nombre normalizado (col D)
      gadmToRoster,    // nombre GADM crudo â†’ { rosterDept, rosterProv } (col H/I) para getZona()
    };
...
// â”€â”€ NormalizaciÃ³n local (sin CamelCase split) â€” para nombres ya normalizados â€”
function norm(str) {
...
// â”€â”€ normGadm: igual que MapaTabâ€™s norm() â€” CamelCase split + preposiciones + abrev. â”€â”€
// DEBE ser idÃ©ntica a la funciÃ³n norm() de MapaTab.js para que gadmKey == idKey en getFeatureKey
function normGadm(str) {
  ...
  return s.toUpperCase()
    ...
    .replace(/\bSTA\b/g, 'SANTA').replace(/\bSTO\b/g, 'SANTO')
    .replace(/\bTTE\b/g, 'TENIENTE').replace(/\bPTE\b/g, 'PRESIDENTE')
    // Mismo fix de preposiciones que MapaTab.js norm(): "NUEVEDE" â†’ "NUEVE DE"
    .replace(/(\w+?)DEL\b/g,'$1 DEL')
    .replace(/(\w+?)DE\b/g,'$1 DE')
    .replace(/(\w+?)LAS\b/g,'$1 LAS')
    .replace(/(\w+?)LOS\b/g,'$1 LOS')
