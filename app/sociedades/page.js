...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/sociedades/zonas').then(r => r.ok ? r.json() : null).then(data => {
  ...
  const handleExportSheets = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/sociedades/export-sociedades', {
        method: 'POST',
  ...
  const handleFetchData = () => {
    ...
    Promise.all([
      fetch('/api/sociedades/funnel', { method: 'POST', body: JSON.stringify({ provincia: provPayload }) }).then(res => res.json()),
      fetch('/api/sociedades/prospectos', { method: 'POST', body: JSON.stringify({ provincia: provPayload }) }).then(res => res.json()),
      fetch('/api/sociedades/usuarios', { method: 'POST', body: JSON.stringify({ busqueda: busquedaUsuarios }) }).then(res => res.json().catch(() => ({error: true})))
