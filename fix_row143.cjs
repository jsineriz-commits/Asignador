// fix_row143.cjs — corrige la fila 143 (SantaMaría de Catamarca incorrectamente mapeada a VALLE VIEJO)
const { google } = require('googleapis');
const SA_EMAIL = 'sheets-bot@minuta-comercial.iam.gserviceaccount.com';
const SA_KEY = `-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n`;
const SHEET_ID = '1KfqTwP2hdeqF_FZSmEEniP1tLvqve8t3p3ZgrUWnk4k';

async function main() {
  const auth = new google.auth.GoogleAuth({
    credentials: { client_email: SA_EMAIL, private_key: SA_KEY },
    scopes: ['https://www.googleapis.com/auth/spreadsheets'],
  });
  const client = await auth.getClient();
  const sheets = google.sheets({ version: 'v4', auth: client });

  // Leer área completa de Catamarca para auditar
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SHEET_ID,
    range: "'GADM - Match Deptos'!A128:I148",
    valueRenderOption: 'FORMATTED_VALUE',
  });
  const rows = res.data.values || [];
  console.log('=== Catamarca area raw ===');
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i];
    console.log(`Row ${128+i}: A="${r[0]||''}" B="${r[1]||''}" C="${r[2]||''}" D="${r[3]||''}" F="${r[5]||''}" G="${r[6]||''}" H="${r[7]||''}" I="${r[8]||''}"`);
  }

  // Fix fila 143: Catamarca/SantaMaría → F y G deben estar vacíos (no en Metabase),
  // H = SANTA MARIA, I = CATAMARCA
  // La fila tiene B=SantaMaría, D=SANTA MARIA, pero F/G fueron incorrectamente llenados con VALLE VIEJO
  console.log('\n📝 Corrigiendo fila 143 (SantaMaría → debe ser SANTA MARIA en Roster, vacío en Metabase)...');
  await sheets.spreadsheets.values.update({
    spreadsheetId: SHEET_ID,
    range: "'GADM - Match Deptos'!F143:I143",
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [['', '', 'SANTA MARIA', 'CATAMARCA']] },
  });
  console.log('✅ Fila 143 corregida: F/G vacíos (sin Metabase), H=SANTA MARIA, I=CATAMARCA');

  // Verificar fila 130 (fJUJUY/Ambato) - esto parece ser un error en col A del sheet original
  console.log('\n⚠️  Nota: Fila 130 tiene A="fJUJUY" (error tipográfico en el sheet GADM original)');
  console.log('   Ese depto (Ambato) H/I están correctos: AMBATO/CATAMARCA');
}
main().catch(console.error);
