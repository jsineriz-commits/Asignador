// fix_santarosa.cjs
const { google } = require('googleapis');
const SA_EMAIL = 'sheets-bot@minuta-comercial.iam.gserviceaccount.com';
const SA_KEY = `-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n`;
const SHEET_ID = '1KfqTwP2hdeqF_FZSmEEniP1tLvqve8t3p3ZgrUWnk4k';

async function main() {
  const auth = new google.auth.GoogleAuth({
    credentials: { client_email: SA_EMAIL, private_key: SA_KEY },
    scopes: ['https://www.googleapis.com/auth/spreadsheets'],
  });
  const client = await auth.getClient();
  const sheets = google.sheets({ version: 'v4', auth: client });

  // Fix fila 143 (SantaRosa de Catamarca): H debe ser SANTA ROSA
  await sheets.spreadsheets.values.update({
    spreadsheetId: SHEET_ID,
    range: "'GADM - Match Deptos'!F143:I143",
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [['', '', 'SANTA ROSA', 'CATAMARCA']] },
  });
  console.log('✅ Fila 143 (SantaRosa) corregida: H=SANTA ROSA, I=CATAMARCA');
  
  // Verificar filas 142-145
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SHEET_ID,
    range: "'GADM - Match Deptos'!A140:I147",
    valueRenderOption: 'FORMATTED_VALUE',
  });
  const rows = res.data.values || [];
  for (const r of rows) {
    console.log(`B="${r[1]||''}" | F="${r[5]||''}" | G="${r[6]||''}" | H="${r[7]||''}" | I="${r[8]||''}"`);
  }
}
main().catch(console.error);
