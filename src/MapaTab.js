...
export default function MapaTab({data188ext,data189,selectedDeptos=[],onDeptoFilter}) {
  ...
  // Auto-fetch Q188
  useEffect(()=>{
    fetch('/api/sociedades/mapa-data')
  ...
  useEffect(()=>{
    Promise.all([
      fetch('/api/sociedades/zonas').then(r=>r.ok?r.json():null).catch(()=>null),
      fetch('/api/sociedades/depto-ids').then(r=>r.ok?r.json():null).catch(()=>null),
    ]).then(([z,ids])=>{
